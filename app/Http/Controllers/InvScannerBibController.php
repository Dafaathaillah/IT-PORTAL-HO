...
class InvScannerBibController extends Controller
{
    ...
    {
        $dataInventory = InvScanner::where('site', 'BIB')->orderBy('scanner_code', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteBib/Scanner/ScannerCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('scannerBib.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('scannerBib.edit', ['id' => $id])
    ...
    public function edit($scannerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPABIBSCN')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMBIBSCN')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteBib/Scanner/ScannerEdit', [
