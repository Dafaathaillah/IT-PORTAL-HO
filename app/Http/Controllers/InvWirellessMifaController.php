...
class InvWirellessMifaController extends Controller
{
    ...
    {
        $dataInventory = InvWirelless::where('site', 'MIFA')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    {
        return Inertia::render('Inventory/SiteMifa/Wirelless/WirellessCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMIFABB' : 'AMMMIFABB';
        ...
        // dd($uniqueString);
        return redirect()->route('wirellessMifa.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('wirellessMifa.edit', ['id' => $id])
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMIFABB')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMIFABB')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteMifa/Wirelless/WirellessEdit', [
            'wirelless' => $wirelless,
