...
class InvComputerIptController extends Controller
{
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvComputer::where('site', 'IPT')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'IPT-PC-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('komputerIpt.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        $aduan_get_data_user = UserAll::where('site', 'IPT')->where('nrp', $params['user_alls_id'])->first();
