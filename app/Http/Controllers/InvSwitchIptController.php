...
class InvSwitchIptController extends Controller
{
    public function index()
    {

        $dataInventory = InvSwitch::where('site', 'IPT')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteIpt/Switch/Switch', ['switch' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteIpt/Switch/SwitchCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAIPTSW' : 'AMMIPTSW';

        // Ambil max_id hanya untuk perusahaan yang dipilih
        $maxId = InvSwitch::Where(function ($query) use ($dataCompany) {
            $query->where('site', 'IPT')->where('inventory_number', 'like', $dataCompany . '%');
        })
            ->orderBy('max_id', 'desc')
            ->first();
        ...
        // dd($uniqueString);
        return redirect()->route('switchIpt.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('switchIpt.edit', ['swId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('switchIpt.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('switchIpt.page')->with([
    ...
    public function edit($swId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAHOSW')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMHOSW')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteIpt/Switch/SwitchEdit', [
            'switch' => $switch,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteIpt/Switch/SwitchDetail', [
            'switchs' => $switch,
        ]);
    }
