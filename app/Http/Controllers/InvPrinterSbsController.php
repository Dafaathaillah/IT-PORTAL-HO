...
class InvPrinterSbsController extends Controller
{
    ...
    {
        $dataInventory = InvPrinter::where('site', 'SBS')->orderBy('printer_code', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteSbs/Printer/PrinterCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('printerSbs.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('printerSbs.edit', ['id' => $id])
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('printerSbs.page')->with([
    ...
    public function edit($printerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPASBSPRT')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMSBSPRT')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteSbs/Printer/PrinterEdit', [
