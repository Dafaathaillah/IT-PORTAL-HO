...
class InvSwitchBgeController extends Controller
{
    public function index()
    {

        $dataInventory = InvSwitch::where('site', 'BGE')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteBge/Switch/Switch', ['switch' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteBge/Switch/SwitchCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPABGESW' : 'AMMBGESW';
        ...
        // dd($uniqueString);
        return redirect()->route('switchBge.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('switchBge.edit', ['swId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('switchBge.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('switchAmi.page')->with([
    ...
    public function edit($swId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPABGESW')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMBGESW')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteBge/Switch/SwitchEdit', [
            'switch' => $switch,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteBge/Switch/SwitchDetail', [
            'switchs' => $switch,
        ]);
    }
