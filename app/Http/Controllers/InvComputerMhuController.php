...
class InvComputerMhuController extends Controller
{
    public function index()
    {
        $dataInventory = InvComputer::with('pengguna')->orderBy('computer_code', 'asc')->where('site', 'MHU')->get();
        ...
        return Inertia::render('Inventory/SiteMhu/Komputer/Komputer', ['komputer' => $dataInventory, 'site' => $site, 'role' => $role, 'department' => $department]);
    }
    ...
    public function create()
    {
        ...
        // $pengguna = UserAll::where('site', 'MHU')->pluck('username')->map(function ($name) {
        //     return ['name' => $name];
        // })->toArray();

        $pengguna = UserAll::where('site', 'MHU')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
        ...
        return Inertia::render('Inventory/SiteMhu/Komputer/KomputerCreate', ['pengguna' => $pengguna, 'department' => $department, 'computer_code' => session('computer_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvComputer::where('site', 'MHU')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'MHU-PC-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('komputerMhu.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        $aduan_get_data_user = UserAll::where('site', 'MHU')->where('nrp', $params['user_alls_id'])->first();
    ...
    public function edit($id)
    {
        ...
        return Inertia::render('Inventory/SiteMhu/Komputer/KomputerEdit', [
            'komputer' => $komputer,
            'model' => $model,
            'processor' => $processor,
            'hdd' => $hdd,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMhu/Komputer/KomputerDetail', [
            'komputer' => $komputer,
            'aduan' => $aduan,
        ]);
    }
