...
class InvLaptopMhuController extends Controller
{
    ...
    {
        $dataInventory = InvLaptop::with('pengguna')->where('site', 'MHU')->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($params['roterx'] == 'index') {
            ...
            $maxId = InvLaptop::where('site', 'MHU')->where('dept', $code_dept->code)->orderBy('max_id', 'desc')->first();
            // dd($maxId);
            ...
            $uniqueString = 'MHU-NB-' . $code_dept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
            ...
            return Inertia::render('Inventory/SiteMhu/Laptop/LaptopCreate', ['inventoryNumber' => $uniqueString, 'pengguna' => $pengguna, 'dept' => $code_dept->code]);
        } else {
