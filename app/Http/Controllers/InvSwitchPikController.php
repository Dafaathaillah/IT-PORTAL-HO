...
class InvSwitchPikController extends Controller
{
    ...
    {
        return Inertia::render('Inventory/SitePik/Switch/SwitchCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAPIKSW' : 'AMMPIKSW';
        ...
        // dd($uniqueString);
        return redirect()->route('switchPik.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('switchPik.edit', ['swId' => $id])
    ...
    public function edit($swId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAPIKSW')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMPIKSW')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SitePik/Switch/SwitchEdit', [
            'switch' => $switch,
