...
class InvLaptopPikController extends Controller
{
    ...
    {
        $dataInventory = InvLaptop::with('pengguna')->orderBy('laptop_code', 'asc')->where('site', 'PIK')->get();
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SitePik/Laptop/LaptopCreate', ['pengguna' => $pengguna, 'department' => $department, 'laptop_code' => session('laptop_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvLaptop::where('site', 'PIK')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'PIK-NB-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('laptopPik.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        if ($existingDataSn) {
            ...
            return Redirect::route('laptopPik.page')->with([
