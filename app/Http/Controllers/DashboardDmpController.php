...
class DashboardDmpController extends Controller
{
    public function index()
    {
        $aduan = Aduan::orderBy('date_of_complaint', 'desc')->where('site', 'DMP')->get();
        $countOpen = Aduan::where('status', 'OPEN')->where('site', 'DMP')->count();
        $countClosed = Aduan::where('status', 'CLOSED')->where('site', 'DMP')->count();
        $countProgress = Aduan::where('status', 'PROGRESS')->where('site', 'DMP')->count();
        $countCancel = Aduan::where('status', 'CANCEL')->where('site', 'DMP')->count();

        // access point
        $countAPreadyused = InvAp::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countAPstandby = InvAp::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countAPBreakdown = InvAp::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countAPScrap = InvAp::where('status', 'SCRAP')->where('site', 'DMP')->count();

        // Switch
        $countSwitchreadyused = InvSwitch::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countSwitchstandby = InvSwitch::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countSwitchBreakdown = InvSwitch::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countSwitchScrap = InvSwitch::where('status', 'SCRAP')->where('site', 'DMP')->count();

        // Wirelless
        $countWirellessreadyused = InvWirelless::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countWirellessstandby = InvWirelless::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countWirellessBreakdown = InvWirelless::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countWirellessScrap = InvWirelless::where('status', 'SCRAP')->where('site', 'DMP')->count();

        // Printer
        $countPrinterreadyused = InvPrinter::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countPrinterstandby = InvPrinter::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countPrinterBreakdown = InvPrinter::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countPrinterScrap = InvPrinter::where('status', 'SCRAP')->where('site', 'DMP')->count();

        // CCTV
        $countCCTVreadyused = InvCctv::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countCCTVstandby = InvCctv::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countCCTVBreakdown = InvCctv::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countCCTVScrap = InvCctv::where('status', 'SCRAP')->where('site', 'DMP')->count();

        // Komputer
        $countKomputerreadyused = InvComputer::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countKomputerstandby = InvComputer::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countKomputerBreakdown = InvComputer::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countKomputerScrap = InvComputer::where('status', 'SCRAP')->where('site', 'DMP')->count();

        // Laptop
        $countLaptopreadyused = InvLaptop::where('status', 'READY_USED')->where('site', 'DMP')->count();
        $countLaptopstandby = InvLaptop::where('status', 'READY_STANDBY')->where('site', 'DMP')->count();
        $countLaptopBreakdown = InvLaptop::where('status', 'BREAKDOWN')->where('site', 'DMP')->count();
        $countLaptopScrap = InvLaptop::where('status', 'SCRAP')->where('site', 'DMP')->count();
        ...
        $countAllDataInspeksiLaptop = InspeksiLaptop::where('site', 'DMP')
        ...
        $countAllDataInspeksiComputer = InspeksiComputer::where('site', 'DMP')
        ...
        $countAllDataInspeksiPrinter = InspeksiPrinter::where('site', 'DMP')
            ->where('year', Carbon::now()->year)
        ...
        $countAllDataInspeksiMT = InspeksiMobileTower::where('site', 'DMP')
            ->where('year', Carbon::now()->year)
            ->where('month', Carbon::now()->month)
            ->where('inspection_status', '!=', '-')
            ->count();
        ...
        $totalAduanAll = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->count();
        if ($totalAduanAll > 0) {
            $totalAduan = $totalAduanAll;

            $aduanTelkomsel = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'TELKOMSEL')->count();
            $aduanTelkomselPercent = ($aduanTelkomsel / $totalAduan) * 100;

            $aduanRadio = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'RADIO')->count();
            $aduanRadioPercent = ($aduanRadio / $totalAduan) * 100;

            $aduanServer = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'SERVER')->count();
            $aduanServerPercent = ($aduanServer / $totalAduan) * 100;

            $aduanSs6 = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'SS6')->count();
            $aduanSs6Percent = ($aduanSs6 / $totalAduan) * 100;

            $aduanWebsite = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'WEBSITE')->count();
            $aduanWebsitePercent = ($aduanWebsite / $totalAduan) * 100;

            $aduanNetwork = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'NETWORK')->count();
            $aduanNetworkPercent = ($aduanNetwork / $totalAduan) * 100;

            $aduanSap = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'SAP')->count();
            $aduanSapPercent = ($aduanSap / $totalAduan) * 100;

            $aduanPcNb = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'PC/NB')->count();
            $aduanPcNbPercent = ($aduanPcNb / $totalAduan) * 100;

            $aduanPrinter = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'PRINTER')->count();
            $aduanPrinterPercent = ($aduanPrinter / $totalAduan) * 100;

            $aduanOther = Aduan::where('site', 'DMP')->whereMonth('created_at', $bulanSekarang)->where('category_name', 'OTHER')->count();
