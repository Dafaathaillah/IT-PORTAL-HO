...
class InvWirellessMacController extends Controller
{
    public function index()
    {
        $dataInventory = InvWirelless::where('site', 'MAC')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteMac/Wirelless/Wirelless', ['wirelless' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteMac/Wirelless/WirellessCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMACBB' : 'AMMMACBB';
        ...
        // dd($uniqueString);
        return redirect()->route('wirellessMac.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('wirellessMac.edit', ['id' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('wirellessMac.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('wirellessMac.page')->with([
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMac/Wirelless/WirellessDetail', [
            'wirelless' => $wirelless,
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMACBB')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMACBB')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteMac/Wirelless/WirellessEdit', [
            'wirelless' => $wirelless,
