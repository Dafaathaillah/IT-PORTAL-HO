...
class InvComputerSksController extends Controller
{
    public function index()
    {
        $dataInventory = InvComputer::with('pengguna')->orderBy('computer_code', 'asc')->where('site', 'SKS')->get();
        ...
        return Inertia::render('Inventory/SiteSks/Komputer/Komputer', ['komputer' => $dataInventory, 'site' => $site, 'role' => $role, 'department' => $department]);
    }
    ...
    public function create()
    {
        ...
        $pengguna = UserAll::where('site', 'SKS')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
        ...
        return Inertia::render('Inventory/SiteSks/Komputer/KomputerCreate', ['pengguna' => $pengguna, 'department' => $department, 'computer_code' => session('computer_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvComputer::where('site', 'SKS')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'SKS-PC-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('komputerSks.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        if ($existingDataSn) {
            ...
            return Redirect::route('komputerSks.page')->with([
        ...
        $aduan_get_data_user = UserAll::where('site', 'SKS')->where('nrp', $params['user_alls_id'])->first();
        ...
        return redirect()->route('komputerSks.page');
    }
    ...
    public function edit($id)
    {
        ...
        // $pengguna_all = UserAll::where('site', 'SKS')->pluck('username')->map(function ($name) {
        //     return ['name' => $name];
        // })->toArray();

        $pengguna_all = UserAll::where('site', 'SKS')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
                'dept' => $item->department,
        ...
        return Inertia::render('Inventory/SiteSks/Komputer/KomputerEdit', [
            'komputer' => $komputer,
            'model' => $model,
            'processor' => $processor,
            'hdd' => $hdd,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteSks/Komputer/KomputerDetail', [
            'komputer' => $komputer,
            'aduan' => $aduan,
        ]);
    }
    ...
    public function update(Request $request)
    {
        if (!empty($request->hasFile('image'))) {
            ...
            $aduan_get_data_user = UserAll::where('site', 'SKS')->where('nrp', $request->user_alls_id)->first();
