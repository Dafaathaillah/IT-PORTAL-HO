...
class InvComputerSksController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($params['roterx'] == 'index') {
            ...
            $maxId = InvComputer::where('site', 'SKS')->where('dept', $code_dept->code)->orderBy('max_id', 'desc')->first();
            // dd($maxId);
            ...
            $uniqueString = 'SKS-PC-' . $code_dept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
            ...
            return Inertia::render('Inventory/SiteSks/Komputer/KomputerCreate', ['inventoryNumber' => $uniqueString, 'pengguna' => $pengguna, 'dept' => $code_dept->code]);
        } else {
            // dd($request->all());
