...
class InvScannerAmiController extends Controller
{
    ...
    {
        $dataInventory = InvScanner::where('site', 'AMI')->orderBy('scanner_code', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteAmi/Scanner/ScannerCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('scannerAmi.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('scannerAmi.edit', ['id' => $id])
   ...
   public function edit($scannerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAAMISCN')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMAMISCN')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteAmi/Scanner/ScannerEdit', [
