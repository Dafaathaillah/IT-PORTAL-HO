...
class InvSwitchBibController extends Controller
{
    public function index()
    {

        $dataInventory = InvSwitch::where('site', 'BIB')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteBib/Switch/Switch', ['switch' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteBib/Switch/SwitchCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPABIBSW' : 'AMMBIBSW';
        ...
        // dd($uniqueString);
        return redirect()->route('switchBib.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('switchBib.edit', ['swId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('switchBib.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('switchBib.page')->with([
    ...
    public function edit($swId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPABIBSW')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMBIBSW')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteBib/Switch/SwitchEdit', [
            'switch' => $switch,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteBib/Switch/SwitchDetail', [
            'switchs' => $switch,
        ]);
    }
