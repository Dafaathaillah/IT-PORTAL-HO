...
class InvSwitchBaController extends Controller
{
    public function index()
    {

        $dataInventory = InvSwitch::where('site', 'BA')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteBa/Switch/Switch', ['switch' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteBa/Switch/SwitchCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPABASW' : 'AMMBASW';
        ...
        // dd($uniqueString);
        return redirect()->route('switchBa.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('switchBa.edit', ['swId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('switchBa.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('switchBa.page')->with([
    ...
    public function edit($swId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPABASW')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMBASW')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteBa/Switch/SwitchEdit', [
            'switch' => $switch,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteBa/Switch/SwitchDetail', [
            'switchs' => $switch,
        ]);
    }
