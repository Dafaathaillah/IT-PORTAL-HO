...
class InvWirellessValeController extends Controller
{
    ...
    {
        return Inertia::render('Inventory/SiteVale/Wirelless/WirellessCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAVIBBB' : 'AMMVIBBB';
        ...
        // dd($uniqueString);
        return redirect()->route('wirellessVale.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('wirellessVale.edit', ['id' => $id])
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAVIBBB')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMVIBBB')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteVale/Wirelless/WirellessEdit', [
            'wirelless' => $wirelless,
