...
class InvCctvSbsController extends Controller
{
    public function index()
    {
        $dataInventory = InvCctv::with('switch')->where('site', 'SBS')->orderBy('cctv_code', 'desc')->get();

        $site = auth()->user()->site;

        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteSbs/Cctv/Cctv', ['cctv' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        $switch = InvSwitch::select('id', 'inventory_number')->where('site', 'SBS')->get();


        return Inertia::render('Inventory/SiteSbs/Cctv/CctvCreate', ['inventory_number' => session('inventory_number') ?? null, 'switch' => $switch]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('cctvSbs.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('cctvSbs.edit', ['id' => $id])
    ...
    public function store(Request $request)
    {
        ...
        // dd($data);
        InvCctv::create($data);
        return redirect()->route('cctvSbs.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('cctvSbs.page')->with([
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPASBSCCTV')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMSBSCCTV')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteSbs/Cctv/CctvEdit', [
            'cctv' => $cctv,
            'switch' => $switch,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteSbs/Cctv/CctvDetail', [
            'cctv' => $cctv,
            'aduan' => $aduan,
        ]);
    }
