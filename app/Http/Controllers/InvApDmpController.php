...
class InvApDmpController extends Controller
{
    public function index()
    {
        $dataInventory = InvAp::where('site', 'DMP')->orderBy('inventory_number', 'desc')->get();
        $site = 'DMP';
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteDmp/AccessPoint/AccessPoint', ['accessPoint' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteDmp/AccessPoint/AccessPointCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPADMPAP' : 'AMMDMPAP';

        // Ambil max_id hanya untuk perusahaan yang dipilih
        $maxId = InvAp::Where(function ($query) use ($dataCompany) {
            $query->where('site', 'DMP')->where('inventory_number', 'like', $dataCompany . '%');
        })
            ->orderBy('max_id', 'desc')
            ->first();
        ...
        // dd($uniqueString);
        return redirect()->route('accessPointDmp.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('accessPointDmp.edit', ['apId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        InvAp::create($data);
        return redirect()->route('accessPointDmp.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        ...
        try {
            ...
            // dd($duplicates);
            return Redirect::route('accessPointDmp.page')->with([
    ...
    public function edit($apId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPADMPAP')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMDMPAP')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteDmp/AccessPoint/AccessPointEdit', [
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteDmp/AccessPoint/AccessPointDetail', [
