...
class InvComputerMipController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('Inventory/SiteMip/Komputer/Komputer', ['komputer' => $dataInventory, 'site' => $site, 'role' => $role, 'department' => $department]);
    }
    ...
    public function create()
    {
        ...
        // $pengguna = UserAll::where('site', 'MIP')->pluck('username')->map(function ($name) {
        //     return ['name' => $name];
        // })->toArray();

        $pengguna = UserAll::where('site', 'MIP')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
        ...
        return Inertia::render('Inventory/SiteMip/Komputer/KomputerCreate', ['pengguna' => $pengguna, 'department' => $department, 'computer_code' => session('computer_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvComputer::where('site', 'MIP')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'MIP-PC-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('komputerMip.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        if ($existingDataSn) {
            ...
            return Redirect::route('komputerMip.page')->with([
        ...
        $aduan_get_data_user = UserAll::where('site', 'MIP')->where('nrp', $params['user_alls_id'])->first();
        ...
        return redirect()->route('komputerMip.page');
    }
    ...
    public function edit($id)
    {
        ...
        // $pengguna_all = UserAll::where('site', 'MIP')->pluck('username')->map(function ($name) {
        //     return ['name' => $name];
        // })->toArray();

        $pengguna_all = UserAll::where('site', 'MIP')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
                'dept' => $item->department,
        ...
        return Inertia::render('Inventory/SiteMip/Komputer/KomputerEdit', [
            'komputer' => $komputer,
            'model' => $model,
            'processor' => $processor,
            'hdd' => $hdd,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMip/Komputer/KomputerDetail', [
            'komputer' => $komputer,
            'aduan' => $aduan,
        ]);
    }
    ...
    public function update(Request $request)
    {
        if (!empty($request->hasFile('image'))) {
            ...
            $aduan_get_data_user = UserAll::where('site', 'MIP')->where('nrp', $request->user_alls_id)->first();
