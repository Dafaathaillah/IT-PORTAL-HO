...
class InvCctvController extends Controller
{
    public function index()
    {
        ...
        $dataInventory = InvCctv::with('switch')->where('site', null)->orWhere('site', 'HO')->orderBy('cctv_code', 'desc')->get();
    ...
    {
        $switch = InvSwitch::select('id', 'inventory_number')->where('site', 'HO')->get();


        return Inertia::render('Inventory/Cctv/CctvCreate', ['inventory_number' => session('inventory_number') ?? null, 'switch' => $switch]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('cctv.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('cctv.edit', ['id' => $id])
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('cctv.page')->with([
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAHOCCTV')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMHOCCTV')) {
            $company = 'AMM';
        }
