...
class InvComputerDmpController extends Controller
{
    public function index()
    {
        $dataInventory = InvComputer::with('pengguna')->orderBy('max_id', 'desc')->where('site', 'DMP')->get();
        ...
        return Inertia::render('Inventory/SiteDmp/Komputer/Komputer', ['komputer' => $dataInventory, 'site' => $site, 'role' => $role, 'department' => $department]);
    }
    ...
    public function create()
    {
        ...
        // $pengguna = UserAll::where('site', 'DMP')->pluck('username')->map(function ($name) {
        //     return ['name' => $name];
        // })->toArray();

        $pengguna = UserAll::where('site', 'DMP')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
        ...
        return Inertia::render('Inventory/SiteDmp/Komputer/KomputerCreate', ['pengguna' => $pengguna, 'department' => $department, 'computer_code' => session('computer_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvComputer::where('site', 'DMP')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'DMP-PC-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('komputerDmp.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        if ($existingDataSn) {
            ...
            return Redirect::route('komputerDmp.page')->with([
        ...
        $aduan_get_data_user = UserAll::where('site', 'DMP')->where('nrp', $params['user_alls_id'])->first();
        ...
        return redirect()->route('komputerDmp.page');
    }
    ...
    public function edit($id)
    {
        ...
        // $pengguna_all = UserAll::where('site', 'DMP')->pluck('username')->map(function ($name) {
        //     return ['name' => $name];
        // })->toArray();

        $pengguna_all = UserAll::where('site', 'DMP')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
                'dept' => $item->department,
        ...
        return Inertia::render('Inventory/SiteDmp/Komputer/KomputerEdit', [
            'komputer' => $komputer,
            'model' => $model,
            'processor' => $processor,
            'hdd' => $hdd,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteDmp/Komputer/KomputerDetail', [
            'komputer' => $komputer,
            'aduan' => $aduan,
        ]);
    }
    ...
    public function update(Request $request)
    {
        if (!empty($request->hasFile('image'))) {
            ...
            $aduan_get_data_user = UserAll::where('site', 'DMP')->where('nrp', $request->user_alls_id)->first();
