...
class InvPrinterMifaController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('Inventory/SiteMifa/Printer/Printer', ['printer' => $dataInventory, 'site' => $site, 'role' => $role]);
    }
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteMifa/Printer/PrinterCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMIFAPRT' : 'AMMMIFAPRT';

        // Ambil max_id hanya untuk perusahaan yang dipilih
        $maxId = InvPrinter::Where(function ($query) use ($dataCompany) {
            $query->where('site', 'MIFA')->where('printer_code', 'like', $dataCompany . '%');
        })
            ->orderBy('max_id', 'desc')
            ->first();
        ...
        // dd($uniqueString);
        return redirect()->route('printerMifa.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('printerMifa.edit', ['id' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('printerMifa.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('printerMifa.page')->with([
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMifa/Printer/PrinterDetail', [
            'printers' => $printer,
        ]);
    }
    ...
    public function edit($printerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMIFAPRT')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMIFAPRT')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteMifa/Printer/PrinterEdit', ['printer' => $printer, 'department' => $department, 'department_select' => $department_select,
