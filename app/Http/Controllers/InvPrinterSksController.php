...
class InvPrinterSksController extends Controller
{
    ...
    {
        $dataInventory = InvPrinter::where('site', 'SKS')->orderBy('printer_code', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteSks/Printer/PrinterCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('printerSks.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('printerSks.edit', ['id' => $id])
    ...
    public function edit($printerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPASKSPRT')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMSKSPRT')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteSks/Printer/PrinterEdit', [
