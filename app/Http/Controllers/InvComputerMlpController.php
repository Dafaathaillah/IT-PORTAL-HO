...
class InvComputerMlpController extends Controller
{
    ...
    {
        $dataInventory = InvComputer::with('pengguna')->orderBy('max_id', 'desc')->where('site', 'MLP')->get();
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvComputer::where('site', 'MLP')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'MLP-PC-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('komputerMlp.create')->with([
