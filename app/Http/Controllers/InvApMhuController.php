...
class InvApMhuController extends Controller
{
    ...
    {
        return Inertia::render('Inventory/SiteMhu/AccessPoint/AccessPointCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMHUAP' : 'AMMMHUAP';

        // Ambil max_id hanya untuk perusahaan yang dipilih
        $maxId = InvAp::Where(function ($query) use ($dataCompany) {
            $query->where('site', 'MHU')->where('inventory_number', 'like', $dataCompany . '%');
        })
            ->orderBy('max_id', 'desc')
            ->first();
        ...
        // dd($uniqueString);
        return redirect()->route('accessPointMhu.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('accessPointMhu.edit', ['apId' => $id])
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('accessPointMhu.page')->with([
    ...
    public function edit($apId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMHUAP')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMHUAP')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteMhu/AccessPoint/AccessPointEdit', [
