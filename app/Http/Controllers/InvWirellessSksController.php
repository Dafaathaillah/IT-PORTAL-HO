...
class InvWirellessSksController extends Controller
{
    public function index()
    {
        $dataInventory = InvWirelless::where('site', 'SKS')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteSks/Wirelless/Wirelless', ['wirelless' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteSks/Wirelless/WirellessCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('wirellessSks.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('wirellessSks.edit', ['id' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('wirellessSks.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('wirellessSks.page')->with([
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteSks/Wirelless/WirellessDetail', [
            'wirelless' => $wirelless,
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPASKSBB')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMSKSBB')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteSks/Wirelless/WirellessEdit', [
            'wirelless' => $wirelless,
