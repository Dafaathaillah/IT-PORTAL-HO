...
class InvScannerMhuController extends Controller
{
    ...
    {
        $dataInventory = InvScanner::where('site', 'MHU')->orderBy('scanner_code', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteMhu/Scanner/ScannerCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('scannerMhu.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('scannerMhu.edit', ['id' => $id])
    ...
    public function edit($scannerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMHUSCN')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMHUSCN')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteMhu/Scanner/ScannerEdit', [
