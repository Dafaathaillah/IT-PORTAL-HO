...
class InvLaptopMlpController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('Inventory/SiteMlp/Laptop/Laptop', ['laptop' => $dataInventory, 'site' => $site, 'role' => $role, 'department' => $department]);
    }
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteMlp/Laptop/LaptopCreate', ['pengguna' => $pengguna, 'department' => $department, 'laptop_code' => session('laptop_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvLaptop::where('site', 'MLP')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'MLP-NB-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('laptopMlp.create')->with([
    ...
    public function store(Request $request)
    {
        ...
        if ($existingDataSn) {
            ...
            return Redirect::route('laptopMlp.page')->with([
        ...
        return redirect()->route('laptopMlp.page');
    }
    ...
    public function edit($id)
    {
        ...
        $pengguna_all = UserAll::where('site', 'MLP')->select('username', 'nrp', 'department')->get()->map(function ($item) {
            return [
                'nrp' => $item->nrp,
                'name' => $item->username,
        ...
        return Inertia::render('Inventory/SiteMlp/Laptop/LaptopEdit', [
            'laptop' => $laptop,
            'model' => $model,
            'processor' => $processor,
            'hdd' => $hdd,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMlp/Laptop/LaptopDetail', [
            'laptop' => $laptop,
            'aduan' => $aduan,
            'inspeksi' => $inspeksi,
    ...
    public function update(Request $request)
    {
        if (!empty($request->hasFile('image'))) {
            ...
            $aduan_get_data_user = UserAll::where('site', 'MLP')->where('nrp', $request->user_alls_id)->first();
