...
class InvSwitchMipController extends Controller
{
    public function index()
    {

        $dataInventory = InvSwitch::where('site', 'MIP')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteMip/Switch/Switch', ['switch' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteMip/Switch/SwitchCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMIPSW' : 'AMMMIPSW';
        ...
        // dd($uniqueString);
        return redirect()->route('switchMip.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('switchMip.edit', ['swId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('switchMip.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('switchMip.page')->with([
    ...
    public function edit($swId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMIPSW')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMIPSW')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteMip/Switch/SwitchEdit', [
            'switch' => $switch,
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMip/Switch/SwitchDetail', [
            'switchs' => $switch,
        ]);
    }
