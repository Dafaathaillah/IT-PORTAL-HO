...
class InvLaptopIptController extends Controller
{
    ...
    {
        $dataInventory = InvLaptop::with('pengguna')->orderBy('max_id', 'desc')->where('site', 'IPT')->get();
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteIpt/Laptop/LaptopCreate', ['pengguna' => $pengguna, 'department' => $department, 'laptop_code' => session('laptop_code') ?? null, 'dept' => session('dept') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $maxId = InvLaptop::where('site', 'IPT')->where('dept', $codeDept->code)->orderBy('max_id', 'desc')->first();
        // dd($maxId);
        ...
        $uniqueString = 'IPT-NB-' . $codeDept->code . '-' . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($codeDept->code);
        return redirect()->route('laptopIpt.create')->with([
