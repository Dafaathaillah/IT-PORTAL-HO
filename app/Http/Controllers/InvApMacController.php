...
class InvApMacController extends Controller
{
    public function index()
    {
        $dataInventory = InvAp::where('site', 'MAC')->orderBy('inventory_number', 'desc')->get();
        $site = 'MAC';
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteMac/AccessPoint/AccessPoint', ['accessPoint' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteMac/AccessPoint/AccessPointCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMACAP' : 'AMMMACAP';

        // Ambil max_id hanya untuk perusahaan yang dipilih
        $maxId = InvAp::Where(function ($query) use ($dataCompany) {
            $query->where('site', 'MAC')->where('inventory_number', 'like', $dataCompany . '%');
        })
            ->orderBy('max_id', 'desc')
            ->first();
        ...
        // dd($uniqueString);
        return redirect()->route('accessPointMac.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('accessPointMac.edit', ['apId' => $id])
    ...
    public function store(Request $request)
    {
        ...
        InvAp::create($data);
        return redirect()->route('accessPointMac.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        ...
        try {
            ...
            // dd($duplicates);
            return Redirect::route('accessPointMac.page')->with([
    ...
    public function edit($apId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMACAP')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMACAP')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteMac/AccessPoint/AccessPointEdit', [
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteMac/AccessPoint/AccessPointDetail', [
