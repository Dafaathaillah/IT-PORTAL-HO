...
class InvWirellessWARAController extends Controller
{
    public function index()
    {
        $dataInventory = InvWirelless::where('site', 'ADW')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteWARA/Wirelless/Wirelless', ['wirelless' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteWARA/Wirelless/WirellessCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        // dd($uniqueString);
        return redirect()->route('wirellessWARA.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('wirellessWARA.edit', ['id' => $id])
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('wirellessWARA.page')->with([
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteWARA/Wirelless/WirellessDetail', [
            'wirelless' => $wirelless,
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAADWBB')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMADWBB')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteWARA/Wirelless/WirellessEdit', [
            'wirelless' => $wirelless,
