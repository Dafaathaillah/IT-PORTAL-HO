...
class InvScannerMifaController extends Controller
{
    ...
    {
        $dataInventory = InvScanner::where('site', 'MIFA')->orderBy('scanner_code', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;
    ...
    public function create()
    {
        ...
        return Inertia::render('Inventory/SiteMifa/Scanner/ScannerCreate', ['inventory_number' => session('inventory_number') ?? null, 'department' => $department]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPAMIFASCN' : 'AMMMIFASCN';

        // Ambil max_id hanya untuk perusahaan yang dipilih
        $maxId = InvScanner::Where(function ($query) use ($dataCompany) {
            $query->where('site', 'MIFA')->where('scanner_code', 'like', $dataCompany . '%');
        })
            ->orderBy('max_id', 'desc')
            ->first();
        ...
        // dd($uniqueString);
        return redirect()->route('scannerMifa.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('scannerMifa.edit', ['id' => $id])
    ...
    public function edit($scannerId)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPAMIFASCN')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMMIFASCN')) {
            $company = 'AMM';
        }
        ...
        return Inertia::render('Inventory/SiteMifa/Scanner/ScannerEdit', [
