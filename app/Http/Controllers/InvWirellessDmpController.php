...
class InvWirellessDmpController extends Controller
{
    public function index()
    {
        $dataInventory = InvWirelless::where('site', 'DMP')->orderBy('inventory_number', 'desc')->get();
        $site = auth()->user()->site;
        $role = auth()->user()->role;

        return Inertia::render('Inventory/SiteDmp/Wirelless/Wirelless', ['wirelless' => $dataInventory, 'site' => $site, 'role' => $role]);
    }

    public function create()
    {
        return Inertia::render('Inventory/SiteDmp/Wirelless/WirellessCreate', ['inventory_number' => session('inventory_number') ?? null]);
    }
    ...
    public function generateCode(Request $request)
    {
        ...
        $prefix = $dataCompany === 'PPA' ? 'PPADMPBB' : 'AMMDMPBB';
        ...
        // dd($uniqueString);
        return redirect()->route('wirellessDmp.create')->with([
    ...
    public function generateCodeEdit(Request $request)
    {
        ...
        $uniqueString = $prefix . str_pad(($maxId % 10000) + 1, 3, '0', STR_PAD_LEFT);
        // dd($uniqueString);
        return redirect()->route('wirellessDmp.edit', ['id' => $id])
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('wirellessDmp.page');
    }
    ...
    public function uploadCsv(Request $request)
    {
        try {
            ...
            // dd($duplicates);
            return Redirect::route('wirellessDmp.page')->with([
    ...
    public function detail($id)
    {
        ...
        return Inertia::render('Inventory/SiteDmp/Wirelless/WirellessDetail', [
            'wirelless' => $wirelless,
        ]);
    }
    ...
    public function edit($id)
    {
        ...
        if (str_starts_with($inventoryNumber, 'PPADMPBB')) {
            $company = 'PPA';
        } elseif (str_starts_with($inventoryNumber, 'AMMDMPBB')) {
            $company = 'AMM';
        }

        return Inertia::render('Inventory/SiteDmp/Wirelless/WirellessEdit', [
            'wirelless' => $wirelless,
