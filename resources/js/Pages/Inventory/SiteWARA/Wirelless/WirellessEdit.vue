...
watch(selectedOptionCompany, async (newVal) => {
    ...
    router.post(
        route("wirellessWARA.generateEdit"),
        { company: newVal, id: props.wirelless.id },
        {
            preserveState: true,
            replace: true,
            onSuccess: (page) => {
...
const update = () => {
    form.put(route("wirellessWARA.update", props.wirelless.id), {
        onSuccess: () => {
...
const save = () => {
    form.post(route("wirellessWARA.store"), {
        onSuccess: () => {
            // Show SweetAlert2 success notification
            Swal.fire({
                title: "Success!",
