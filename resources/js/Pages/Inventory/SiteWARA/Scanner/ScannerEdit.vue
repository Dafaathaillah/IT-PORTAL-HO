...
watch(selectedOptionCompany, async (newVal) => {
    ...
    router.post(
        route("scannerWARA.generateEdit"),
        { company: newVal, id: props.scanner.id },
        {
            preserveState: true,
            replace: true,
            onSuccess: (page) => {
                console.log(page);
                form.scanner_code = page.props.inventory_number;
            },
        }
    );
});
...
const update = () => {
    ...
    form.post(route("scannerWARA.update", props.scanner.id), {
        onSuccess: () => {
...
const save = () => {
    form.post(route("scannerWARA.store"), {
        onSuccess: () => {
            // Show SweetAlert2 success notification
            Swal.fire({
                title: "Success!",
