...
const getEncryptedYear = () => {
    ...
    router.post(
        route("encrypt.year"),
        { year: selectedYear, site: "ADW", pic: selectPic },
        {
            onSuccess: ({ props }) => {
...
const editData = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("inspeksiLaptopWARA.edit", { id: id }));
        }
    });
};
...
const detailData = (id) => {
    form.get(route("inspeksiLaptopWARA.detail", { id: id }));
};

const processData = (id) => {
    form.get(route("inspeksiLaptopWARA.process", { id: id }));
};
