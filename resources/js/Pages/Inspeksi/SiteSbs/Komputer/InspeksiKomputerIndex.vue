...
const editData = (id) => {
    form.get(route("inspeksiKomputerSbs.inspection", { id: id }));
};
...
const editDataInspeksi = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("inspeksiKomputerSbs.edit", { id: id }));
        }
    });
};
...
const getEncryptedYear = () => {
    ...
    router.post(
        ...
        { year: selectedYear, quarter: selectedQuarter, site: "SBS" },
        {
            onSuccess: ({ props }) => {
...
const detailData = (id) => {
    form.get(route("inspeksiKomputerSbs.detail", { id: id }));
};
