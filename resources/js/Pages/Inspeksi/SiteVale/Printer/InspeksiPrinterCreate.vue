...
watch(temuanEnabled, (enabled) => {
    if (!enabled) {
        ...
        if (temuanFile.value) temuanFile.value.value = "";
        if (tindakanFile.value) tindakanFile.value.value = "";
    }
});
...
const save = () => {
    ...
    // === VALIDASI SAAT TEMUAN ENABLED ===
    if (temuanEnabled.value) {
        // 1. Temuan wajib
        ...
        // 2. Foto temuan wajib
        if (!form.findings_image && !temuanFile.value?.files?.length) {
        ...
        // 4. Status temuan wajib
        if (!form.findings_status) {
            errors.value.findings_status = "Status temuan wajib dipilih.";
        }

        // 5. Jika tindakan TIDAK kosong → foto tindakan WAJIB
        if (form.tindakan && form.tindakan.trim() !== "") {
            if (!form.action_image && !tindakanFile.value?.files?.length) {
    ...
    form.post(route("inspeksiPrinterVale.store"), {
        onSuccess: () => {
                        ...
                        >
                            <div class="flex items-center">
                                <h4 class="mb-0 font-black dark:text-white/80">
                                    Form Inspeksi dan Perawatan Printer
                                </h4>
                            </div>
                        </div>
