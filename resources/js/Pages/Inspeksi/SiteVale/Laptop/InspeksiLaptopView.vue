...
const getEncryptedYear = () => {
    ...
    router.post(
        route("encrypt.year"),
        { year: selectedYear, site: "VIB", pic: selectPic },
        {
            onSuccess: ({ props }) => {
...
const editData = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("inspeksiLaptopVale.edit", { id: id }));
        }
    });
};
...
const detailData = (id) => {
    form.get(route("inspeksiLaptopVale.detail", { id: id }));
};

const processData = (id) => {
    form.get(route("inspeksiLaptopVale.process", { id: id }));
};
