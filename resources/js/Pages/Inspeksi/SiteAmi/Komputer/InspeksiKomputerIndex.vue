...
const editData = (id) => {
    form.get(route("inspeksiKomputerAmi.inspection", { id: id }));
};
...
const editDataInspeksi = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("inspeksiKomputerAmi.edit", { id: id }));
        }
    });
};
...
const getEncryptedYear = () => {
    ...
    router.post(
        ...
        { year: selectedYear, quarter: selectedQuarter, site: "AMI" },
        {
            onSuccess: ({ props }) => {
...
const detailData = (id) => {
    form.get(route("inspeksiKomputerAmi.detail", { id: id }));
};
