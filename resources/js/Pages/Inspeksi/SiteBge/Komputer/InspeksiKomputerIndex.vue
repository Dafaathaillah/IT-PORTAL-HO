...
const editData = (id) => {
    form.get(route("inspeksiKomputerBge.inspection", { id: id }));
};
...
const editDataInspeksi = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.get(route("inspeksiKomputerBge.edit", { id: id }));
        }
    });
};
...
const getEncryptedYear = () => {
    ...
    router.post(
        ...
        { year: selectedYear, quarter: selectedQuarter, site: "BGE" },
        {
            onSuccess: ({ props }) => {
...
const detailData = (id) => {
    form.get(route("inspeksiKomputerBge.detail", { id: id }));
};
